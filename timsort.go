...
type timSortHandler[T Comparable[T]] struct {
 ...
 */
func newTimSort[T Comparable[T]](a []T) (h *timSortHandler[T]) {
	h = new(timSortHandler[T])
...
func TimSort[T Comparable[T]](a []T) {
	lo := 0
	hi := len(a)
	nRemaining := hi
	...
	if nRemaining < minMerge {
		...
		err = binarySort(a, lo, hi, lo+initRunLen)
		if err != nil {
			panic(err)
		}
	}
 ...
 */
func binarySort[T Comparable[T]](a []T, lo, hi, start int) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscending[T Comparable[T]](a []T, lo, hi int) (int, error) {
	...
	if a[runHi].Compare(a[lo]) < 0 { // Descending
		runHi++

		for runHi < hi && a[runHi].Compare(a[runHi-1]) < 0 {
			runHi++
		}
 ...
 * entry to the method.
 */
func (h *timSortHandler[T]) mergeCollapse() (err error) {
	for h.stackSize > 1 {
		n := h.stackSize - 2
 ...
 */
func (h *timSortHandler[T]) mergeForceCollapse() (err error) {
	for h.stackSize > 1 {
		n := h.stackSize - 2
 ...
 */
func (h *timSortHandler[T]) mergeAt(i int) (err error) {
	if h.stackSize < 2 {
 ...
 *    should follow it.
 */
func gallopLeft[T Comparable[T]](key T, a []T, base, len, hint int) (int, error) {
	...
	if a[base+hint].Compare(key) < 0 {
		// Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
		maxOfs := len - hint
		for ofs < maxOfs && a[base+hint+ofs].Compare(key) < 0 {
			lastOfs = ofs
			ofs = (ofs << 1) + 1
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRight[T Comparable[T]](key T, a []T, base, len, hint int) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
	...
	ofs := 1
	lastOfs := 0
	if key.Compare(a[base+hint]) < 0 {
		// Gallop left until a[b+hint - ofs] <= key < a[b+hint - lastOfs]
		maxOfs := hint + 1
		for ofs < maxOfs && key.Compare(a[base+hint-ofs]) < 0 {
			lastOfs = ofs
			ofs = (ofs << 1) + 1
			if ofs <= 0 { // int overflow
				ofs = maxOfs
 ...
 */
func (h *timSortHandler[T]) mergeLo(base1, len1, base2, len2 int) (err error) {
	...
	minGallop := h.minGallop // Use local variable for performance
...
outer:
	for {
		...
		for {
			...
			if a[cursor2].Compare(tmp[cursor1]) < 0 {
				a[dest] = a[cursor2]
				dest++
				cursor2++
				count2++
 ...
 * @param len2  length of second run to be merged (must be > 0)
 */
func (h *timSortHandler[T]) mergeHi(base1, len1, base2, len2 int) (err error) {
	if len1 <= 0 || len2 <= 0 || base1+len1 != base2 {
		return errors.New("len1 > 0 && len2 > 0 && base1 + len1 == base2;")
	}
...
outer:
	for {
		...
		for {
			...
			if tmp[cursor2].Compare(a[cursor1]) < 0 {
				a[dest] = a[cursor1]
				dest--
				cursor1--
				count1++
